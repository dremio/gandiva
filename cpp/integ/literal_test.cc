...
TEST_F(TestLiteral, TestNullLiteralInIf) {
  ...
  auto a_gt_5 = TreeExprBuilder::MakeFunction("greater_than", {node_a, literal_5},
                                              arrow::boolean());
  auto literal_null = TreeExprBuilder::MakeNull(arrow::int32());
  auto if_node = TreeExprBuilder::MakeIf(a_gt_5, literal_5, literal_null, arrow::int32());
  ...
  // Create a row-batch with some sample data
  int num_records = 4;
  auto array_a = MakeArrowArrayInt32({6, 15, -15, 17}, {true, true, true, false});

  // expected output
  auto exp = MakeArrowArrayInt32({5, 5, 0, 0}, {true, true, false, false});
